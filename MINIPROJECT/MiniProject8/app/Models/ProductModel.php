...
class ProductModel extends Model
{
    ...
    {
        return $this->select('categories.name as category_name, COUNT(products.id) as product_count, ROUND((COUNT(products.id) * 100.0 / (SELECT COUNT(*) FROM products)), 2) as percentage')
                    ->join('categories', 'categories.id = products.category_id')
    ...
    {
        $query = $this->select("TO_CHAR(created_at, 'YYYY-MM') as period, COUNT(id) as product_count")
                    ->where("created_at >= NOW() - INTERVAL '12 months'", null, false)
                    ->groupBy("period")
                    ->orderBy("period")
                    ->get()
                    ->getResultArray();

        // Buat daftar 12 bulan terakhir dengan default 0
        $months = [];
        for ($i = 11; $i >= 0; $i--) {
            $date = date('Y-m', strtotime("-$i months"));
            $months[$date] = ['period' => date('F Y', strtotime($date)), 'product_count' => 0];
        }

        // Isi dengan data dari database
        foreach ($query as $row) {
            if (isset($months[$row['period']])) {
                $months[$row['period']]['product_count'] = $row['product_count'];
            }
        }

        return array_values($months); // Hanya ambil nilai, tanpa kunci array
    }

    public function getTopCategories()
    {
        return $this->select('categories.name as category_name, COUNT(products.id) as product_count')
                    ->join('categories', 'categories.id = products.category_id')
                    ->groupBy('categories.id')
    ...
    public function exportProductExcel($category = null)
    {
        $query = $this->getProductsWithCategory();
