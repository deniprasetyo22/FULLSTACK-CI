...
<div class="flex flex-col space-y-4 p-6 bg-white shadow rounded-lg h-full">
    ...
    <form action="<?= base_url('admin/update-enrollment/' . $enrollment->id) ?>" method="post" id="formData" novalidate>
        <?= csrf_field() ?>
        ...
        <div class="mb-4">
            ...
            <select name="student_id" id="student_id" data-pristine-required
                ...
                class="w-full border <?= session('errors.student_id') ? 'border-red-500' : 'border-gray-300' ?> rounded p-2">
        ...
        <div class="mb-4">
            ...
            <select name="course_id" id="course_id" data-pristine-required
                ...
                class="w-full border <?= session('errors.course_id') ? 'border-red-500' : 'border-gray-300' ?> rounded p-2">
        ...
        <div class="mb-4">
            ...
            <select name="semester" id="semester" data-pristine-required
                ...
                class="w-full border <?= session('errors.semester') ? 'border-red-500' : 'border-gray-300' ?> rounded p-2">
                <option value="" disabled>-- Select Semester --</option>
                <?php foreach ($semester as $s) : ?>
                <option value="<?= $s ?>" <?= old('semester', $enrollment->semester) == $s ? 'selected' : '' ?>>
                    <?= $s ?>
                </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="status" id="status" data-pristine-required data-pristine-required-message="Status is required"
                class="w-full border <?= session('errors.status') ? 'border-red-500' : 'border-gray-300' ?> rounded p-2">
